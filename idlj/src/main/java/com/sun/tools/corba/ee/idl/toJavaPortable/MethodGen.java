 ...
 *
 **/
public class MethodGen implements com.sun.tools.corba.ee.idl.MethodGen
{
  ...
  protected void writeParmType (SymtabEntry parm, int passType)
  {
    ...
    {
      parm = com.sun.tools.corba.ee.idl.toJavaPortable.Util.typeOf(parm);
      stream.print (com.sun.tools.corba.ee.idl.toJavaPortable.Util.holderName(parm));
    }
    else // passType is `in'
      // <f46082.51> Remove -stateful feature; javaStatefulName() obsolete.
      //stream.print (Util.javaStatefulName (parm));
      stream.print (com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(parm));
  } // writeParmType
  ...
  protected void writeDispatchCall ()
  {
    ...
    SymtabEntry mtype = com.sun.tools.corba.ee.idl.toJavaPortable.Util.typeOf(m.type());
    ...
    while (parms.hasMoreElements ())
    {
      ...
      if (passType == ParameterEntry.In)
        com.sun.tools.corba.ee.idl.toJavaPortable.Util.writeInitializer(indent, name, "", type, writeInputStreamRead("in", type), stream);

      else // the parm is a holder
      {
        String holderName = com.sun.tools.corba.ee.idl.toJavaPortable.Util.holderName(type);
    ...
    if (mtype != null)
      com.sun.tools.corba.ee.idl.toJavaPortable.Util.writeInitializer(indent, "$result", "", mtype, stream);
    ...
    if (m.exceptions ().size () > 0)
    {
      ...
      while (exceptions.hasMoreElements ())
      {
        ...
        String fullName = com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaQualifiedName(exc);
        stream.println (indent + "} catch (" +  fullName + " $ex) {");
        indent = indent + "  ";
        stream.println (indent + "out = $rh.createExceptionReply ();");
        stream.println (indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(exc, true) + ".write (out, $ex);"); // <d61056>
      }
  ...
  protected void writeStubBody ( String className )
  {
    ...
    String methodName = com.sun.tools.corba.ee.idl.toJavaPortable.Util.stripLeadingUnderscores(m.name());
    if (m instanceof AttributeEntry)
    {
      if (m.type () == null)          // if it's a modifier
    ...
    if (mtype != null)
      com.sun.tools.corba.ee.idl.toJavaPortable.Util.writeInitializer(FOUR_INDENT, "$result", "", mtype,
  ...
  private void writeInsert (String indent, String target, String source, SymtabEntry type, PrintWriter stream)
  {
    ...
    if (type instanceof PrimitiveEntry)
    {
      ...
      stream.println (indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".insert (" + source + ", " + target + ");"); // <d61056>
  } // writeInsert
  ...
  private void writeType (String indent, String name, SymtabEntry type, PrintWriter stream)
  {
    ...
    else if (type instanceof StringEntry)
    {
      ...
      if (e == null)
        stream.println (indent + name + " (org.omg.CORBA.ORB.init ().create_" + type.name () + "_tc (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(e) + "));");
     else
        stream.println (indent + name + " (org.omg.CORBA.ORB.init ().create_" + type.name () + "_tc (0));");
    }
    else
      stream.println (indent + name + '(' + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".type ());"); // <d61056>
  } // writeType
  ...
  private void writeExtract (String indent, String target, String source, SymtabEntry type, PrintWriter stream)
  {
    if (type instanceof PrimitiveEntry)
    {
      ...
      stream.println (indent + target + " = " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".extract (" + source + ");"); // <d61056>
  } // writeExtract
  ...
  private String writeExtract (String source, SymtabEntry type)
  {
    ...
    if (type instanceof PrimitiveEntry)
    {
      ...
      extract = com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".extract (" + source + ')'; // <d61056>
    return extract;
  } // writeExtract
  ...
  private void writeOutputStreamWrite (String indent, String oStream, String name, SymtabEntry type, PrintWriter stream)
  {
    ...
    else if (type instanceof ValueBoxEntry)
    {
      ...
      if (mType instanceof PrimitiveEntry && name.endsWith (".value"))
        stream.println (com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", "  // <d61056>
        + " new " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaQualifiedName(type) + " (" + name + "));"); //<d60929>
      else
        stream.println (com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", " + name + ");"); //<d60929> // <d61056>
    }
    else if (type instanceof ValueEntry)
        stream.println (com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", " + name + ");"); //<d60929> // <d61056>
    else
      stream.println (com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", " + name + ");"); // <d61056>
