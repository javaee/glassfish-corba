 ...
 *
 **/
public class SequenceGen implements com.sun.tools.corba.ee.idl.SequenceGen, com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator
{
  /**
  ...
  public int helperType (int index, String indent, TCOffsets tcoffsets, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (offsetOfType >= 0)
    {
      ...
      if (maxSize == null)
        ...
        stream.println (indent + name + " = org.omg.CORBA.ORB.init ().create_recursive_sequence_tc (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(maxSize) + ", " + (offsetOfType - tcoffsets.currentOffset ()) + ");");
    ...
    else
    {
      ...
      tcoffsets.set (entry);
      index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)entry.type ().generator ()).helperType (index + 1, indent, tcoffsets, name, entry.type (), stream);
      Expression maxSize = ((SequenceEntry)entry).maxSize ();
      if (maxSize == null)
        stream.println (indent + name + " = org.omg.CORBA.ORB.init ().create_sequence_tc (0, " + name + ");");
      else
        stream.println (indent + name + " = org.omg.CORBA.ORB.init ().create_sequence_tc (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(maxSize) + ", " + name + ");");
    }
  ...
  public int type (int index, String indent, TCOffsets tcoffsets, String name, SymtabEntry entry, PrintWriter stream) {
    ...
    else
    {
      // This is a normal sequence
      tcoffsets.set (entry);
      index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)entry.type ().generator ()).type (index + 1, indent, tcoffsets, name, entry.type (), stream);
      Expression maxSize = ((SequenceEntry)entry).maxSize ();
      if (maxSize == null)
        stream.println (indent + name + " = org.omg.CORBA.ORB.init ().create_sequence_tc (0, " + name + ");");
      else
  ...
  public int read (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (seq.maxSize () != null)
    {
      stream.println (indent + "if (" + length + " > (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(seq.maxSize()) + "))");
      stream.println (indent + "  throw new org.omg.CORBA.MARSHAL (0, org.omg.CORBA.CompletionStatus.COMPLETED_MAYBE);");
    }
    String seqOfName;
    try
    {
      seqOfName = com.sun.tools.corba.ee.idl.toJavaPortable.Util.sansArrayInfo ((String) seq.dynamicVariable(com.sun.tools.corba.ee.idl.toJavaPortable.Compile.typedefInfo));
    }
    ...
    SymtabEntry seqOfEntry = (SymtabEntry) com.sun.tools.corba.ee.idl.toJavaPortable.Util.symbolTable.get (seqOfName.replace ('.', '/'));
    if (seqOfEntry != null && seqOfEntry instanceof InterfaceEntry && ((InterfaceEntry)seqOfEntry).state () != null)
      ...
      seqOfName = com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName((InterfaceEntry) seqOfEntry);
    ...
    if (seq.type () instanceof PrimitiveEntry)
      ...
      { // special case for ValueBox: if name is "xxx tmp", drop xxx
        ...
        stream.println (indent + "istream.read_" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.collapseName(entry.type().name()) + "_array (" + varName + ", 0, " + length + ");");
      }
    ...
    else if (entry.type () instanceof SequenceEntry)
    {
      ...
      index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)seq.type ().generator ()).read (index, indent + "  ", name + '[' + loopIndex + ']', seq.type (), stream);
      stream.println (indent + '}');
    }
    else
    { // special case for ValueBox: if name is "xxx tmp", drop xxx
      ...
      stream.println (indent + "  " + varName + '[' + loopIndex + "] = " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(seq.type(), true) + ".read (istream);"); // <d61056>
    }
    return index;
  } // read
  ...
  public int write (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (seq.maxSize () != null)
    {
      stream.println (indent + "if (" + name + ".length > (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(seq.maxSize()) + "))");
      stream.println (indent + "  throw new org.omg.CORBA.MARSHAL (0, org.omg.CORBA.CompletionStatus.COMPLETED_MAYBE);");
    }
    ...
    if (entry.type () instanceof PrimitiveEntry)
          ...
          entry.type ().name ().equals ("Principal"))
      {
        ...
        stream.println (indent + "ostream.write_" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.collapseName(entry.type().name()) + "_array (" + name + ", 0, " + name + ".length);");
    else if (entry.type () instanceof StringEntry)
    {
      String loopIndex = "_i" + index++;
      stream.println (indent + "for (int " + loopIndex + " = 0;" + loopIndex + " < " + name + ".length; ++" + loopIndex + ')');
    ...
    else if (entry.type () instanceof SequenceEntry)
    {
      ...
      stream.println (indent + "for (int " + loopIndex + " = 0;" + loopIndex + " < " + name + ".length; ++" + loopIndex + ')');
      stream.println (indent + '{');
      index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)seq.type ().generator ()).write (index, indent + "  ", name + '[' + loopIndex + ']', seq.type (), stream);
      stream.println (indent + '}');
    }
    else
    {
      String loopIndex = "_i" + index++;
      stream.println (indent + "for (int " + loopIndex + " = 0;" + loopIndex + " < " + name + ".length; ++" + loopIndex + ')');
      stream.println (indent + "  " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(seq.type(), true) + ".write (ostream, " + name + '[' + loopIndex + "]);"); // <d61056>
    }
    return index;
  } // write
