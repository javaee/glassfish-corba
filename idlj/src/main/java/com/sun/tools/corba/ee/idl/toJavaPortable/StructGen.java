...
public class StructGen implements com.sun.tools.corba.ee.idl.StructGen, com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator
{
  ...
  public int helperType (int index, String indent, com.sun.tools.corba.ee.idl.toJavaPortable.TCOffsets tcoffsets, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    for (int i = 0; i < s.members ().size (); ++i)
    {
      ...
      index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)member.generator ()).type (index, indent, innerOffsets, tcOfMembers, member, stream);
    ...
    stream.println (indent + name + " = org.omg.CORBA.ORB.init ().create_" + (thisIsReallyAnException ? "exception" : "struct") + "_tc (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(s, true) + ".id (), \"" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.stripLeadingUnderscores(entry.name()) + "\", " + membersName + ");"); // <d61056>
    return index;
  } // helperType
  ...
  public int read (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    while (e.hasMoreElements ())
    {
      ...
      if (!member.arrayInfo ().isEmpty () || mType instanceof SequenceEntry ||
        ...
        index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)member.generator ()).read (index, indent, name + '.' + member.name (), member, stream);
      ...
      {
        // call read_value instead of Helper.read for the value
        stream.println (indent + name + '.' + member.name () + " = (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(mType) + ") ((org.omg.CORBA_2_3.portable.InputStream)istream).read_value (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(mType, false) + ".get_instance ());"); // <d60929> // <d61056>
      }
      else
        stream.println (indent + name + '.' + member.name () + " = " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(member.type(), true) + ".read (istream);"); // <d61056>
    }
    return index;
  } // read
  ...
  public int write (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    for (int i = 0; i < members.size (); ++i)
    {
      ...
      if (!member.arrayInfo ().isEmpty () || mType instanceof SequenceEntry ||
        ...
        index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)member.generator ()).write (index, "    ", name + '.' + member.name (), member, stream);
      ...
      //                everything else
        ...
        stream.println (indent + "((org.omg.CORBA_2_3.portable.OutputStream)ostream).write_value ((java.io.Serializable) " // <d60929>
                        + name + '.' + member.name () + ", "
                        + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(member.type(), true) // <d61056>
                        + ".get_instance ());"); // <d61056>
      }
      else
        stream.println (indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(member.type(), true) + ".write (ostream, " + name + '.' + member.name () + ");"); // <d61056>
    }
    return index;
  } // write
