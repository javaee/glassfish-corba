...
public class UnionGen implements com.sun.tools.corba.ee.idl.UnionGen, com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator
{
  ...
  private void writeBranchMethods (PrintWriter stream, UnionEntry u, UnionBranch branch, int i)
  {
    ...
    if (branch.labels.size () > 0 || branch.isDefault)
    {
      ...
      stream.println ("  public void " + branch.typedef.name () + " (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(utype) + " discriminator, " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(branch.typedef) + " value)");
      stream.println ("  {");
    ...
    // Write verifyXXX
    stream.println ();
    stream.println ("  private void verify" + branch.typedef.name () + " (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(utype) + " discriminator)");
    stream.println ("  {");
  ...
  public int write (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    SymtabEntry utype = com.sun.tools.corba.ee.idl.toJavaPortable.Util.typeOf(u.type());
    if (utype instanceof PrimitiveEntry)
      index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)utype.generator ()).write (index, indent, name + ".discriminator ()", utype, stream);
    else
      stream.println (indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(utype, true) + ".write (ostream, " + name + ".discriminator ());"); // <d61056>
    if (utype.name ().equals ("boolean"))
  ...
  private int writeBranch (int index, String indent, String name, TypedefEntry entry, PrintWriter stream)
  {
    ...
    if (!entry.arrayInfo ().isEmpty () || type instanceof SequenceEntry || type instanceof PrimitiveEntry || type instanceof StringEntry)
      index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator)entry.generator ()).write (index, indent, name + '.' + entry.name () + " ()", entry, stream);
    else
      stream.println (indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".write (ostream, " + name + '.' + entry.name () + " ());"); // <d61056>
    return index;
  } // writeBranch
