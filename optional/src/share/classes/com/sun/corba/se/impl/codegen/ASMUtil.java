...
// import com.sun.corba.se.impl.codegen.constantpool.ConstantPool ;
...
*/
	...
	// get debug options
	boolean dumpConstantPool = Boolean.parseBoolean(
	    options.getProperty( Wrapper.DUMP_CONSTANT_POOL )) ;
	boolean dumpAfterSetupVisitor = Boolean.parseBoolean(
	    options.getProperty( Wrapper.DUMP_AFTER_SETUP_VISITOR )) ;
	boolean traceByteCodeGeneration = Boolean.parseBoolean(
	    options.getProperty( Wrapper.TRACE_BYTE_CODE_GENERATION )) ;
	boolean useAsmVerifier = Boolean.parseBoolean( 
	    options.getProperty( Wrapper.USE_ASM_VERIFIER )) ;
	String classGenDir = options.getProperty( 
