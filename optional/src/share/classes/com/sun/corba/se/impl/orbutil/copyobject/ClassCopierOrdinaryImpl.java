...
//******************************************************************************
    ...
    {
	return Boolean.getBoolean( "com.sun.corba.se.ORBUseBCELReflectiveCopyobject" ) ;
	...
	if (copier == null) {
		...
		copier = (ClassFieldCopier) AccessController.doPrivileged( 
			...
			public Object run() throws ReflectiveCopyException {
			    if (useBCELCopier())
				return makeClassFieldCopierUnsafeBCELImpl( 
...
//******************************************************************************
	...
	if (cons == null) {
		...
		cls.getName() + "Copier" ;
	    final BCELCopierGenerator generator = 
		new BCELCopierGenerator( className, cls ) ;
		...
		ReflectiveCopyException rce = new ReflectiveCopyException( 
		    "Could not access unsafe BCEL copier constructor" ) ;
		rce.initCause( exc ) ;
		throw rce ;
	    } ;
	...
	try {
	    ...
	    ReflectiveCopyException rce = new ReflectiveCopyException( 
		"Could not create unsafe BCEL copier" ) ;
	    rce.initCause( exc ) ;
	    throw rce ;
	}
