...
import com.sun.corba.se.impl.presentation.rmi.bcel.ProxyCreator ;
import com.sun.corba.se.impl.presentation.rmi.bcel.CodegenProxyCreator ;
...
public class StubFactoryBCELImpl extends StubFactoryDynamicBase  
{
    ...
    public StubFactoryBCELImpl( PresentationManager pm,
	...
	synchronized (classData) {
		...
		final IDLNameTranslator nt = classData.getIDLNameTranslator() ;
			...
			theClass.getName() ) ; 
                final String baseClassName = BCELStubBase.class.getName() ;
		...
		final ProxyClassCreator creator = USE_BCEL ?
		    new ProxyCreator( stubClassName, baseClassName, 
			interfaces, methods ) :
		    new CodegenProxyCreator( stubClassName, baseClassName,
			interfaces, methods ) ;
	...
	try {
		...
		new PrivilegedExceptionAction<BCELStubBase>() {
		    public BCELStubBase run() throws Exception {
			return BCELStubBase.class.cast(
