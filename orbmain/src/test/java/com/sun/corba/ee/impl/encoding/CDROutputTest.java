...
public class CDROutputTest extends EncodingTestBase {
    ...
    @Test
    public void canWriteLittleEndianUTF16CharIn_1_0WithLegacyORB() throws IOException {
        useV1_0();
        useLittleEndian();
        setOrbVersion(ORBVersionFactory.getOLD());
        setWCharEncoding(UTF_16);

        getOutputObject().write_wchar('\u5634');
        expectByteArray(0x34, 0x56);
    }
    ...
    public void can_read_little_endian_integers() {
        setMessageBody(0x04, pad(),
                       ...
                       pad(), pad(),// for long
                       0x83,     2, 1, 0,// long
                       0x71,  0xfd, FF, FF,// ulong
                       pad(), pad(), pad(), pad(),// for long_long
                         7, 1, 0x80, 0, 0, 1, 0, 0);// long long
        useLittleEndian();
