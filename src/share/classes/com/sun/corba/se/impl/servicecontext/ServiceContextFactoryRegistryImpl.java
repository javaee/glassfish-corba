    ...
    implements ServiceContextFactoryRegistry
{
    ...
    {
	Integer key = new Integer( factory.getId() ) ;
	if (scMap.get(key) == null)
	    scMap.put( key, factory ) ;
	else
	    wrapper.registerDuplicateServiceContext() ; // BAD_PARAM
    }

    public ServiceContext.Factory find( int scId )
    {
	Integer key = new Integer( scId ) ;
	ServiceContext.Factory result = 
	    (ServiceContext.Factory)scMap.get( key ) ;
	return result ;
    }
}
