...
public class Skeleton implements AuxGen
{
  ...
  protected void writeMethodTable ()
  {
    ...
    while (e.hasMoreElements ())
    {
      ...
      {
        stream.println ("    _methods.put (\"_get_" + Util.stripLeadingUnderscores (method.name ()) + "\", " + (++count) + ");");
        if (!((AttributeEntry)method).readOnly ())
          stream.println ("    _methods.put (\"_set_" + Util.stripLeadingUnderscores (method.name ()) + "\", " + (++count) + ");");
      }
      else
        stream.println ("    _methods.put (\"" + Util.stripLeadingUnderscores (method.name ()) + "\", " + (++count) + ");");
    }
    stream.println ("  }");
    stream.println ();
  } // writeMethodTable
