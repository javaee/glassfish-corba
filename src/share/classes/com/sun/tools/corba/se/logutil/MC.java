...
public class MC {
  ...
  private void writeFactoryMethod(String className, String groupName,
                                  IndentingPrintWriter pw) {
    pw.indent();
    pw.println( "private static final LogWrapperFactory FACTORY = new LogWrapperFactory() {");
    ...
    pw.println( "} ;" );
    pw.println();
    pw.printMsg( "public static final String EXCEPTION_GROUP_NAME = \"@\" ;", groupName ) ;
    pw.println() ;
    ...
    pw.undent();
    pw.undent();
    pw.println( "EXCEPTION_GROUP_NAME, FACTORY ) ;" ) ;
    pw.undent();
  ...
  private void writeMethodStatusCause(String groupName, String exceptionName,
    ...
    pw.println();
    pw.indent();
    pw.printMsg( "if (getLogger().isLoggable( Level.@ )) {", logLevel);
    if (numParams > 0) {
