...
#{
/*
 * #[ interface-name ]#.java #[ version-string ]# #[ (java.util.Date. ) ]#
 *
 * Copyright (c) 2010 Oracle and/or its affiliates.  All rights reserved.
 *
...
@ORBException( omgException=#[ omg-exception ]#, group=#[ group-id ]# )
...
(define (write-annotated-method level log-id message return-type method-name num-args)
    ...
    @Message( "#[ message ]#" )
    #[ return-type ]# #[ method-name ]#( #[ (make-arg-list num-args) ]# ) ;
    }#
)
...
(define (generate-logex-interface omg-exception infile outdіr)
    (let* (
	...
	(exceptions (cadddr obj))
	(methods (get-method-declarations group exceptions))
...
(define (get-method-declarations exception-group-name exception-data)
    (let* (
	...
	(exception-list (cdr exception-data)))
	    (str-cat
			...
			(write-annotated-method level value explanation exception method-name num-args)))
		    exception-list))))
