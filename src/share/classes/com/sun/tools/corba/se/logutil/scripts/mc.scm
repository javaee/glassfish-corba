...
; Write out an annotated interface that represents the exceptions data in
;
(define (write-annotated-interface package interface-name
    id-prefix omg-exception group-id methods )
    (let 
        ((omg-exception-string (if omg-exception "true" "false")))
#{
/*
 * #[ interface-name ]#.java 
 * generated by mc.scm version #[ version-string ]# #[ (java.util.Date. ) ]#
 *
 * Copyright (c) 2010 Oracle and/or its affiliates.  All rights reserved.
 *
...
@ExceptionWrapper( idPrefix="#[ id-prefix ]#" )
@ORBException( omgException=#[ omg-exception-string ]#, group=#[ group-id ]# )
public interface #[ interface-name ]# {
    #[ interface-name ]# self = WrapperGenerator.makeWrapper( 
        #[ interface-name ]#.class, CorbaExtension.self ) ;
...
; We will write methods out assuming that the default CSValue (NO) is fine, then
; fix the few exceptions by hand.
; All arguments will be initially typed as String; this also needs to be changed
; by hand.  Note that the number of args will be taken from the message, and
; named arg0...argn.  Better names should also be chosen.
; @Chain will not be generated; those methods that need it will be handled manually.
(define (write-annotated-method level log-id message return-type method-name num-args)
    (begin
        (dprint (.concat "write-annotated-method: method-name=" method-name))
	(let ((arg-list (make-arg-list num-args)))
    #{
    @Log( level=LogLevel.#[ level ]#, id=#[ log-id ]# )
    @Message( "#[ message ]#" )
...
(define (test-wam) (write-annotated-method "WARNING" 42 "This is a test"
    "UNKNOWN" "fooIsMyName" 3 ))

; Generated a String that looks like
; String arg0, ..., String argn-1
; where n is num-args.
(define (make-arg-list num-args)
    (cond
        ((eqv? num-args 1)
            "String arg0")
        ((eqv? num-args 0)
        ...
        (else
	    (let* ( (m1 (- num-args 1))
		    (na-string (.concat ", String arg" (Integer.toString m1))))
		  (.concat (make-arg-list m1) na-string)))))

(define exception-groups '(ORBUtil Activation Naming Interceptors POA IOR Util))

(define (get-list-pos elem list) 
    (if (null? list)
        -1
	(if (.equals elem (car list))
	    0
	    (+ 1 (get-list-pos elem (cdr list))))))

(define (get-list-pos elem list)
    (if (null? list)
        -1
	(if (eqv? elem (car list))
	    0
	    (let ((val (get-list-pos elem (cdr list))))
	         (if (eqv? val -1)
		     -1
		     (+ 1 val))))))

(define (test-get-list-pos)
     (let ((list '(1 2 3 4 5 6 7)))
	 (begin
	     (display (get-list-pos 1 list))
	     (display (get-list-pos 4 list))
	     (display (get-list-pos 14 list)))))
	      
(define (get-group-id name)
    (get-list-pos name exception-groups))

(define (generate-logex-interface omg-exception infile outdir)
    (let* (
	(obj (read (open-input-file infile)))
	(package (car obj))
	(class (cadr obj))
	(group (caddr obj))
	(group-id (get-group-id group))
	(exceptions (cadddr obj))
	(methods (str-cat 
	    (map
                (lambda (elem) (get-method-declarations elem))
		exceptions)))
	(file (FileOutputStream. (string-append outdir java.io.File.separator$ class ".java")))
	(ps (PrintStream. file))
	)
        (begin
	    (.print ps (write-annotated-interface package class "IOP" omg-exception group-id methods))
	    (.close ps) 
	)))

(define (test-gen-anno) (generate-logex-interface #f "IOR.mc" "."))

; Return a string of method declarations corresponding to 
; (exception (name value level explanation)...)
(define (get-method-declarations exception-data)
    (let* (
	(exception (car exception-data))
	(exception-list (cdr exception-data)))
            (begin 
	        (dprint exception)
		(dprint exception-list)
		(str-cat
		    (map2
			(lambda (elem)
			    (let* (
				(name (symbol->string (car elem)))
				(value (cadr elem))
				(level (caddr elem))
				(explanation (cadddr elem))
				(num-args (StringUtil.countArgs explanation))
				(method-name (StringUtil.toMixedCase name)))
			    (begin
				(dprint name)
				(dprint value)
				(dprint level)
				...
				(dprint num-args)
				(dprint method-name)
				(write-annotated-method level value explanation exception 
				    method-name num-args))))
			exception-list)))))

(define (test-get-method-decls) 
    (get-method-declarations 
        '(UNKNOWN
	    (FIRST_TEST 1 WARNING "String 1")
	    (SECOND_TEST 2 WARNING "String 2"))))

(define (fold func start) 
    (lambda (list)
	(if (null? list)
	    start
	    (func (car list) ((fold func start) (cdr list)) ))))

(define (sum list) ((fold + 0) list))

(define (str-cat list) ((fold .concat "") list))

(define (trace-anno)
    (begin
        (trace 'map2)
	(trace 'generate-logex-interface)
	(trace 'test-gen-anno)
        (trace 'get-method-declarations)
	(trace 'write-annotated-method)
	(trace 'test-get-method-decls)))

(define (map2 func list)
    (begin
	(if (null? list)
	    ()
	    (cons (func (car list)) (map2 func (cdr list))))))

(define (test-map2)
    (map2 (lambda (x) (* x x)) '(1 2 3 4)))
